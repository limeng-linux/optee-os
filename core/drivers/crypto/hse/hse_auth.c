...
{
	TEE_Result res;
	struct hse_authenc_ctx *hse_ctx = NULL;
	struct hse_auth_tpl *alg_tpl;
...
static TEE_Result hse_authenc_init(struct drvcrypt_authenc_init *dinit)
{
	...
	if (dinit->aad_len) {
		hse_ctx->aad = hse_buf_alloc(dinit->aad_len);
		if (!hse_ctx->aad) {
			res = TEE_ERROR_OUT_OF_MEMORY;
			goto out_err;
		}
	...
	hse_ctx->iv = hse_buf_init(dinit->nonce.data, dinit->nonce.length);
	if (!hse_ctx->iv) {
		res = TEE_ERROR_OUT_OF_MEMORY;
		goto out_err;
	}
...
hse_authenc_update_aad(struct drvcrypt_authenc_update_aad *update)
{
	...
	uint32_t aad_len = hse_buf_get_size(hse_ctx->aad);

	if (update->aad.length + hse_ctx->saved_aad_len > aad_len)
		return TEE_ERROR_BAD_PARAMETERS;
...
hse_authenc_start_stream(struct drvcrypt_authenc_update_payload *d)
{
	...
	srv_desc.hseSrv.aeadReq.ivLength = hse_buf_get_size(hse_ctx->iv);
	srv_desc.hseSrv.aeadReq.pIV = hse_buf_get_paddr(hse_ctx->iv);
					  ...
					  hseCipherDir_t direction)
{
	...
	struct hse_buf *input = NULL, *output = NULL, *tag = NULL;
	uint32_t input_len;
...
{
	TEE_Result ret;
	struct hse_authenc_ctx *dst = dst_ctx;
	struct hse_authenc_ctx *src = src_ctx;
	struct drvcrypt_authenc_init init_params;
	uint8_t *iv = NULL;
	uint32_t iv_size, aad_size;
	...
	iv_size = hse_buf_get_size(src->iv);
	iv = malloc(iv_size);
	if (!iv) {
		ret = TEE_ERROR_OUT_OF_MEMORY;
		goto err;
	}
	hse_buf_get_data(src->iv, iv, iv_size, 0);

	aad_size = hse_buf_get_size(src->aad);

	init_params = (struct drvcrypt_authenc_init) {
		.ctx = dst,
	...
	ret = hse_authenc_init(&init_params);
	if (ret != TEE_SUCCESS)
		goto err;

	dst->stream_started = src->stream_started;
	if (src->aad) {
		hse_buf_copy(dst->aad, src->aad, aad_size);
	...
	return;
err:
	EMSG("HSE Auth Copy operation failed with return code: 0x%x", ret);
}
