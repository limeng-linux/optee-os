...
static TEE_Result hse_cipher_alloc_ctx(void **ctx, uint32_t algo)
{
	...
	struct hse_cipher_ctx *hse_ctx = NULL;
	const struct hse_cipher_tpl *alg_tpl;
...
out_err:
	...
	EMSG("Cipher context allocation failed with err: 0x%x", ret);
	return ret;
}
...
static TEE_Result hse_cipher_init(struct drvcrypt_cipher_init *dinit)
{
	...
	if (dinit->iv.data && dinit->iv.length &&
	    dinit->iv.length == alg->ivsize) {
		ctx->iv = hse_buf_init(dinit->iv.data, dinit->iv.length);
		if (!ctx->iv) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto err;
		}
...
err:
	...
	if (ret != TEE_SUCCESS)
		EMSG("Cipher initialization failed with err 0x%x", ret);
...
static TEE_Result hse_cipher_update(struct drvcrypt_cipher_update *dupdate)
{
	...
	size_t src_len, dst_len = dupdate->dst.length,
	       blocksize = alg->blocksize, blks_num, blks_len;
	...
	srv_desc.hseSrv.symCipherReq.pIV = hse_buf_get_paddr(ctx->iv);
	...
	if (ret != TEE_SUCCESS) {
		EMSG("HSE Cipher Request failed");
		goto out;
	}
	...
	case HSE_CIPHER_BLOCK_MODE_CBC:
		...
		hse_buf_put_data(ctx->iv, last_block, alg->ivsize, 0);
		break;

	case HSE_CIPHER_BLOCK_MODE_CTR:
		blks_num = src_len / blocksize;
		rem_size = src_len % blocksize;
		blks_len = src_len - rem_size;

		hse_ctr_inc(ctx->iv, blks_num, blocksize);

		memcpy(prev_data, src_data + blks_len, rem_size);
...
out:
	...
	if (ret != TEE_SUCCESS)
		EMSG("Cipher update operation failed with err: 0x%x", ret);
...
static void hse_cipher_copy_state(void *dst_ctx, void *src_ctx)
{
	...
	if (!dst->iv) {
		dst->iv = hse_buf_alloc(alg->ivsize);
		if (!dst->iv) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	hse_buf_copy(dst->iv, src->iv, alg->ivsize);

	return;
err:
	EMSG("Cipher copy operation failed with err: 0x%x", ret);
}
