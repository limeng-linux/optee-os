				     ...
				     hseKeyFlags_t flag)
{
	...
	if (e_len > HSE_MAX_RSA_PUB_EXP_SIZE ||
	    n_len < min_keylen || n_len > max_keylen ||
	    d_len < min_keylen || d_len > max_keylen) {
...
out:
	...
	if (ret != TEE_SUCCESS)
		EMSG("HSE RSA Keypair request failed with err 0x%x", ret);
	return ret;
}
...
out:
	...
	if (ret != TEE_SUCCESS)
		EMSG("HSE RSA Public Key request failed with err 0x%x", ret);
	return ret;
}

static TEE_Result rsa_enc_sw_fallback(struct drvcrypt_rsa_ed *rsa_data)
{
	DMSG("Falling back to sw implementation for RSA encryption");

	switch (rsa_data->rsa_id) {
	case DRVCRYPT_RSA_NOPAD:
		return sw_crypto_acipher_rsanopad_encrypt
		       (rsa_data->key.key, rsa_data->message.data,
			rsa_data->message.length, rsa_data->cipher.data,
			&rsa_data->cipher.length);
		break;
	case DRVCRYPT_RSA_OAEP:
	case DRVCRYPT_RSA_PKCS_V1_5:
		return sw_crypto_acipher_rsaes_encrypt
		       (rsa_data->algo, rsa_data->key.key, rsa_data->label.data,
			rsa_data->label.length, rsa_data->message.data,
			rsa_data->message.length, rsa_data->cipher.data,
			&rsa_data->cipher.length);
	default:
		return TEE_ERROR_BAD_PARAMETERS;
...
static TEE_Result rsa_dec_sw_fallback(struct drvcrypt_rsa_ed *rsa_data)
{
	DMSG("Falling back to sw implementation for RSA decryption");

	switch (rsa_data->rsa_id) {
	case DRVCRYPT_RSA_NOPAD:
		return sw_crypto_acipher_rsanopad_decrypt
		       (rsa_data->key.key, rsa_data->cipher.data,
			rsa_data->cipher.length, rsa_data->message.data,
			&rsa_data->message.length);
		break;
	case DRVCRYPT_RSA_OAEP:
	case DRVCRYPT_RSA_PKCS_V1_5:
		return sw_crypto_acipher_rsaes_decrypt
		       (rsa_data->algo, rsa_data->key.key, rsa_data->label.data,
			rsa_data->label.length, rsa_data->cipher.data,
			rsa_data->cipher.length, rsa_data->message.data,
			&rsa_data->message.length);
	default:
		return TEE_ERROR_BAD_PARAMETERS;
...
static TEE_Result rsa_sign_sw_fallback(struct drvcrypt_rsa_ssa *ssa_data)
{
	DMSG("Falling back to sw implementation for RSA signing");

	return sw_crypto_acipher_rsassa_sign
	       (ssa_data->algo, ssa_data->key.key, ssa_data->salt_len,
		ssa_data->message.data, ssa_data->message.length,
		ssa_data->signature.data, &ssa_data->signature.length);
}

static TEE_Result rsa_verify_sw_fallback(struct drvcrypt_rsa_ssa *ssa_data)
{
	DMSG("Falling back to sw implementation for RSA verify");

	return sw_crypto_acipher_rsassa_verify
	       (ssa_data->algo, ssa_data->key.key, ssa_data->salt_len,
		ssa_data->message.data, ssa_data->message.length,
		ssa_data->signature.data, ssa_data->signature.length);
}
				  ...
				  hseCipherDir_t direction)
{
	...
	hseRsaCipherScheme_t *cipher_sch = &rsa_cipher_req->rsaScheme;
	TEE_Result res;
	...
	if (res == TEE_ERROR_NOT_SUPPORTED) {
		res = (direction == HSE_CIPHER_DIR_ENCRYPT) ?
		rsa_enc_sw_fallback(rsa_data) :
		rsa_dec_sw_fallback(rsa_data);
	...
	if (res != TEE_SUCCESS) {
		EMSG("HSE RSA Cipher Request failed with ret=0x%x", res);
		goto out;
	}
...
out:
	...
	if (res != TEE_SUCCESS)
		EMSG("HSE RSA Encrypt operation failed with err 0x%x", res);
				   ...
				   hseAuthDir_t direction)
{
	...
	if (res == TEE_ERROR_NOT_SUPPORTED) {
		res = (direction == HSE_AUTH_DIR_GENERATE) ?
		rsa_sign_sw_fallback(ssa_data) :
		rsa_verify_sw_fallback(ssa_data);
...
out:
	...
	if (res != TEE_SUCCESS) {
		...
		EMSG("HSE RSA sign request failed with err 0x%x", res);
	}

	return res;
}

static TEE_Result hse_gen_keypair(struct rsa_keypair *key,
				  size_t size_bits)
{
	/* HSE cannot export RSA keypairs. Fallback to sw implementation */
