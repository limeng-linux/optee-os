 ...
 */
TEE_Result
hse_buf_copy(struct hse_buf *dst, struct hse_buf *src, size_t size)
{
	if (!src || !dst)
		return TEE_ERROR_BAD_PARAMETERS;
	...
	cache_operation(TEE_CACHEINVALIDATE, src->data, size);

	memcpy(dst->data, src->data, size);

	cache_operation(TEE_CACHEFLUSH, src->data, size);
	cache_operation(TEE_CACHEFLUSH, dst->data, size);
	return TEE_SUCCESS;
}
...
/**
 * hse_ctr_inc - in AES counter mode, each time a block is encrypted/decrypted,
